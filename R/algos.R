...
sp.actual <- c(
  df$sp.actual,     4743,  4705,  4689,  4697,  4764,  4757,  4783,  4780,  4784,
      ...
      5464,  5436,  5522,  5447,  5347,  5186,  5240,  5200,  5319,  5344)
...
fid.val <- c(
  df$fid.val,     226255,222123,222288,222370,226164,226085,226787,226918,226111,
    ...
    230492,231121,234023,230335,227764,223875,226465,227110,231277,230029) #
...
f1k.in <- c(
  df$f1k.in,       36528, 36528, 36528, 36528, 36528, 36528, 36528, 36528, 36528,
     ...
     46377, 46377, 46377, 47106, 47106, 47106, 47106, 47106, 47106, 47106) # (1062.63)
f1k.val <- c(
  df$f1k.val,      48986, 48283, 47658, 47523, 47500, 48441, 48481, 48834, 48889,
     ...
     67821, 67816, 67574, 69644, 68506, 67126, 65277, 65948, 65475, 67067) #
...
self.mng.val <- c(
  df$self.mng.val, 16169, 15852, 15887, 15861, 16152, 16065, 16106, 16048, 15949,
     ...
     16488, 16492, 16625, 16299, 15946, 15616, 15772, 15698, 16035, 15978)  #
...
et.in <- c(
  df$et.in,        86278, 86278, 86278, 86278, 86278, 86278, 86278, 86278, 86588,
     ...
     88739, 89028, 89028, 89028, 89028, 89254, 89254, 89254, 89254, 89254) #
et.val <-c(
  df$et.val,      182169,178447,178828,178839,181753,181913,181774,181776,181803,
    ...
    179820,181277,183210,180633,176273,174758,176789,178243,181266,179645) #
...
sim1 <- (c(
    ...
    103929,104266,105237,103533,101957, 99949,101244,100744,102747,102785)
  / 100000)
sim1.res <- 9027
...
sim2 <- (c(
    ...
    111472,116798,120334,116236,112745,109882,111628,110277,115633,113297)
  / 100000)
sim2.res <- 23290
...
extr <- (c(
    ...
    100678,100494,100666,100437,100358, 98689, 98696, 99736, 97676, 97695, 97824,
     98220, 96418, 96543, 93702, 91196, 90755, 92744, 90577, 95241, 95138)
  / 100000)
idx.only <- (c(
	                                                 rep(NA, 129), 100000, 99866,
     99970,101426,101818,100279, 98481, 97567, 98504, 99147, 96220, 93994, 94906,
     95447, 94981, 96484, 95792, 92730, 88453, 88016, 87403, 89615, 91371)
  / 100000)
...
sim3 <- (c(
    ...
    113342,118055,121977,117278,113981,110631,112268,110201,115378,114264)
  / 100000)
sim3.res <- 24046
...
sim4 <- (c(
    ...
    102540,103044,104648,102637,101417, 99866,100512,101333, 96173, 95937, 98692,
     97515, 96021, 98963, 94534, 90575, 87070, 88895, 87692, 91417, 91140)
  / 100000)
sim4.res <- 19288
...
sim5 <- (c(
    ...
    105346,104116,106826,100338, 95865, 90840, 92163, 91706, 95009, 90574)
  / 100000)
sim5.res <- 14540
