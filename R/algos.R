...
sp.actual <- c(
  df$sp.actual,    3824,  3853,  3808,  3895,  3892,  3919,  3970,  3987,  3999,  
     ...
     4329,  4378,  4377,  4396,  4450,  4456,  4447,  4412,  4399)
...
fid.val <- c(
  df$fid.val,    172350,175073,172978,176119,179159,182181,185241,188878,190980,
   ...
   211744,212727,212829,212458,213398,213960,213811,212068,212763) #
...
f1k.in <- c(
  df$f1k.in,      18031, 18031, 18031, 18031, 18031, 18031, 18031, 18031, 18031,
    ...
    29023, 29023, 29023, 29023, 29023, 29023, 29023, 29023, 29950) # (1133.49)
f1k.val <- c(
  df$f1k.val,     17787, 17715, 17856, 17766, 18063, 18219, 18434, 18750, 18928,
    ...
    34866, 34728, 35201, 35268, 35187, 35507, 35513, 35475, 36359) #
...
self.mng.val <- c(
  df$self.mng.val, 9189,  9333,  9243,  9351,  9514,  9655,  9818,  9938, 10033,
    ...
    15711, 15863, 15886, 15797, 15850, 15900, 15862, 15733, 15798)
...
et.in <- c(
  df$et.in,       76890, 78443, 78443, 78443, 78443, 78443, 78443, 78443, 78443,
    ...
    78952, 78952, 78952, 78952, 79400, 79400, 79400, 79400, 79400) #
et.val <-c(
  df$et.val,     123916,128480,126726,128449,130010,131945,134582,135693,137333,
   ...
   150081,149574,149699,149059,150891,151589,151256,150002,150225) #
...
rsi <- (c(
   ...
   111870,111503,112668,111215,113304,113670,114084,110376,111140)
  / 100000)
...
adl <- (c(
   ...
   100272,100549,101189,100018,101336,101954,102029, 99131, 99474)
  / 100000)
...
(fid.amt <- fid.val[length(fid.val)])                # [179159 215151]
(et.amt <- et.val[length(et.val)])                   # [130010 152885]
(f1k.total <- f1k.val[length(f1k.val)])              # [ 18063  34291] 
(self.mng.amt <- self.mng.val[length(self.mng.val)]) # [  9514  15900]
