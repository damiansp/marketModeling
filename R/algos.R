...
# Buy when low:
# AAPL, BROS, GOOG, MSFT, TSLA, INTG ($33)
...
SPActual <- c(df$SPActual,
    ...
    4523,  4524,  4537,  4535,  4514,  4493,  4459)
...
FidVal <- c(df$FidVal,
  ...
  112180,113632,113336,113576,111785,112347,111865)
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
...
f1kIn <- c(df$f1kIn,
   ...
   93285, 93285, 93285, 93285, 93285, 93285, 93285) #
f1kVal <- c(df$f1kVal,
  ...
  146142,146142,146551,146551,145349,145349,145013) #
payment <- 1018.67
n <- length(f1kIn)
...
OHIn <- c(df$OHIn,
   ...
   42552, 42874, 42874, 42874, 42874, 42874, 42874) #
OHVal <- c(df$OHVal, 
  ...
  101622,101246,103242,102694,103325,102648,102961,100355,101452, 87037, 86693,
   ...
   96806, 98546, 98505, 99118, 97763, 98736, 98238) # 
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
m1 <- (c(
  ...
  117336,110919,108642,108221,101922,104631,106850,105531, 98774, 94278, 93620,
   ...
   98093, 98299, 99140, 99848,101014, 99445, 99682, 99038)
  / 100000)
qp.start <- 92
m1.name <- 'BuyNHold'
...
m2 <- (c(
  ...
  106966,106520,108051,107668,108095,105836,106558,105886)
  / 100000) 
...
m3.raw <- c(
  ...
  105487,105340,107042,107443,107928,105601,105803,105309)
m3 <- m3.raw / 100000
m3.name <- 'CashInLow'
#len <- length(m3)
...
m4.raw <- c(
  ...
  101283,101490,102260,102439,104144,101691,101668,100958)
m4 <- m4.raw / 100000
m4.name <- 'CashInHigh'
len <- length(m4)
...
m5 <- (c(
  ...
  105363,105457,106680,107205,108625,106709,107124,106896)
  / 100000)
m5.name <- '30/70'  
...
m6 <- (c(
  ...
  108285,109107,109680,110727,111329,110030,110023,109432)
  / 100000)
m6.name <- 'rsi each'  
...
m7 <- (c(
  ...
  100999,100935,101636,101615,102230,100958,101478,101015)
  / 100000)
m7.name <- 'rsi overall'  
...
# Wed    
wil10 <- c(1, 0.9999, 0.9504)
nas10 <- c(1, 1, 0.0100)

# Thu
wil20 <- c(0.8845, 0.0144, 0)
nas20 <- c(1, 1, 0.9961)

# Fri
nya35 <- c(1, 0.9849, 0.0107)
nas35 <- c(0.9989, 1, 1)
