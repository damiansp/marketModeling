...
sp.actual <- c(
  df$sp.actual,     4743,  4705,  4689,  4697,  4764,  4757,  4783,  4780,  4784,
      ...
      5071,  5072,  5048,  5100,  5116,  5036,  5018,  5064,  5128)
...
fid.val <- c(
  df$fid.val,     226255,222123,222288,222370,226164,226085,226787,226918,226111,
    ...
    224858,225305,224032,224974,226345,222858,222309,225158,227389) #
...
f1k.in <- c(
  df$f1k.in,       36528, 36528, 36528, 36528, 36528, 36528, 36528, 36528, 36528,
     ...
     41938, 41938, 41938, 41938, 41938, 41938, 41938, 42720, 42720) # (1062.63)
f1k.val <- c(
  df$f1k.val,      48986, 48283, 47658, 47523, 47500, 48441, 48481, 48834, 48889,
     ...
     56370, 57153, 57229, 57020, 57613, 57942, 57016, 57902, 58548) #
...
self.mng.val <- c(
  df$self.mng.val, 16169, 15852, 15887, 15861, 16152, 16065, 16106, 16048, 15949,
     ...
     15136, 15205, 15126, 15293, 15480, 15259, 15239, 15511, 15673)  #
...
et.in <- c(
  df$et.in,        86278, 86278, 86278, 86278, 86278, 86278, 86278, 86278, 86588,
     ...
     87077, 87077, 87077, 87077, 87125, 87125, 87125, 87125, 87125) # 
et.val <-c(
  df$et.val,      182169,178447,178828,178839,181753,181913,181774,181776,181803,
    ...
    174774,175161,173555,175121,176185,173538,173590,175874,178244) #
...
sim1 <- (c(
    ...
    103257,102868,102540,102360,102660,103013,103286,102430,100591,100223, 99133,
     ...
     99191, 99400, 99387,100031, 99257, 97828, 98185, 99118, 99744)
  / 100000)
sim1.res <- 20337
sim1.name <- 'adelaide'
...
sim2 <- (c(
    ...
    103814,103012,102045,101976,102074,103215,103715,102167, 99869, 98982, 97437,
     ...
     98994, 99690, 98507, 98996, 97989, 95867, 96738, 98294, 99758)
  / 100000)
sim2.res <- 24525
sim2.name <- 'aei'
...
sim3 <- (c(
    ...
    100489,101338,100515, 99660, 98712, 96340, 96238, 97956, 99296)
  / 100000)
sim3.res <- 1701
sim3.name <- 'simsims'
...
(sim1[length(sim1)] * 100000); (sim1.res)
(sim2[length(sim2)] * 100000); (sim2.res)
(sim3[length(sim3)] * 100000); (sim3.res)
(pct.invested)
