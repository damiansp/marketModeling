...
wlsh.osc <- c(
 ...
 -528,-528,-516,-498,-473,-441,-424,-414,-376,-350,-321,-287,-259,-240,-219)
...
FidVal <- c(df$FidVal,
  ...
  245734,249364,242369,247895,257702,251481,245407,249188) #
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
...
f1kIn <- c(df$f1kIn,
  ...
  0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,
    ...
    2023,  2023,  2023,  2023,  2023,  2023,  2023,  2023) # 961.47
f1kVal <- c(df$f1kVal,
  ...
  147813,146496,145095,141741,141741,141741,     1,     1,     1,     1,     1,
    ...
    2895,  2900,  2911,  2908,  2940,  2988,  2954,  2911) #
n <- length(f1kIn)
...
OHIn <- c(df$OHIn,
    ...
    58978, 58978, 58978, 58978, 58978, 58978, 58978, 59760) #
OHVal <- c(df$OHVal, 
   ...
   126322,127162,124866,126801,131445,130528,127861,130695) #
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
m1 <- (c(
  100000, 99734, 97021, 92607, 93071, 91736, 92132, 94069, 93531, 90683, 90091,
   ...
   87026, 85860, 87058, 85346, 86812, 90121, 88040, 86501, 87299)
  / 100000)
qp.start <- 92
m1.name <- 'sim1'
...
m2 <- (c(
  100000, 99519, 96234, 91473, 91946, 90337, 90702, 92795, 92113, 88699, 88134,
   ...
   83072, 81761, 82686, 81217, 82571, 85155, 83529, 82094, 82940)
  / 100000) 
m2.name <- 'sim2'
...
m3.raw <- c(
  100000, 99890, 96310, 91049, 91683, 89443, 89246, 91155, 90781, 88037, 87453, 
   ...
   88753, 87410, 88721, 86792, 88646, 91518, 89937, 88551, 89545)
m3 <- m3.raw / 100000
m3.name <- 'sim3'
...
m4.raw <- c(
  100000, 99289, 96063, 91314, 91644, 90074, 90327, 92360, 91830, 88380, 88025,
   ...
   84860, 83303, 84431, 83013, 84238, 86767, 85113, 84015, 84779)
m4 <- m4.raw / 100000
m4.name <- 'sim4'
...
m5 <- (c(
  100000, 99775, 96651, 91870, 92422, 91307, 92182, 94394, 93768, 89786, 89177,
   ...
   89302, 88138, 88974, 87142, 88853, 92280, 90186, 88372, 89101)
  / 100000)
m5.name <- 'sim5'  
...
m6 <- (c(
  100000,100012, 98204, 96007, 96400, 95686, 96971, 97980, 98605, 95083, 95146,
   ...
   95327, 93939, 95411, 93709, 96084,100081, 98029, 96120, 97397)
  / 100000)
m6.name <- 'sim6'  
...
m7 <- (c(
  100000, 99901, 98262, 96147, 96334, 95340, 95359, 96898, 96645, 95102, 94744,
   ...
   92723, 91477, 92380, 90661, 92244, 95161, 93805, 92365, 92990)
  / 100000)
m7.name <- 'sim7'  
...
(fid.amt <- FidVal[length(FidVal)])   # [80533, 257702]
(oh.amt <- OHVal[length(OHVal)])      # [92495, 131445] # E*Trade
(f1k.total <- f1kVal[length(f1kVal)]) # [  847,   2988] [2-param]
