...
SPActual <- c(df$SPActual,
  ...
  3930,4024,4008,4089,3924,3901,3901)
...
FidVal <- c(df$FidVal,
  ...
  192151,187333,191359,199792,195092,198413,193282,196613,195702) #
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
...
f1kIn <- c(df$f1kIn,
  ...
  0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,
    ...
    4129,  4129,  4129,  4129,  4129,  4129,  4129,  4936,  4936) #
f1kVal <- c(df$f1kVal,
  ...
  147813,146496,145095,141741,141741,141741,     1,     1,     1,     1,     1,
    ...
    4790,  4837,  4728,  4712,  4887,  4820,  4922,  5657,  5652) #
n <- length(f1kIn)
...
OHIn <- c(df$OHIn,
    ...
    63568, 63568, 63568, 63568, 64053, 64053, 64053, 64053, 64053) # 657 + 202
OHVal <-c(df$OHVal,    
   ...
   109388,106990,108934,114344,112948,114852,111417,113169,113265) #
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
m1 <- (c(
  100000, 99734, 97021, 92607, 93071, 91736, 92132, 94069, 93531, 90683, 90091,
   ...
   61297, 59691, 56166, 58338, 63260, 61025, 63151, 60198, 62075, 61473)
  / 100000)
#qp.start <- 92
m1.name <- 'sim1'
...
m2 <- (c(
  100000, 99519, 96234, 91473, 91946, 90337, 90702, 92795, 92113, 88699, 88134,
   ...
   58921, 57771, 54938, 56562, 60454, 58552, 60800, 58448, 59895, 59358)
  / 100000) 
m2.name <- 'sim2'
...
m3.raw <- c(
  100000, 99890, 96310, 91049, 91683, 89443, 89246, 91155, 90781, 88037, 87453, 
   ...
   65646, 64752, 62474, 63326, 67463, 66242, 69585, 67034, 68403, 67820)
m3 <- m3.raw / 100000
m3.name <- 'sim3'
...
m4.raw <- c(
  100000, 99289, 96063, 91314, 91644, 90074, 90327, 92360, 91830, 88380, 88025,
   ...
   60320, 59258, 56414, 57910, 61900, 60036, 61948, 59727, 61018, 60880)
m4 <- m4.raw / 100000
m4.name <- 'sim4'
...
m5 <- (c(
  100000, 99775, 96651, 91870, 92422, 91307, 92182, 94394, 93768, 89786, 89177,
   ...
   63066, 62220, 59209, 61155, 66022, 63790, 65607, 62780, 64470, 64079)
  / 100000)
m5.name <- 'sim5'  
...
m6 <- (c(
  100000,100012, 98204, 96007, 96400, 95686, 96971, 97980, 98605, 95083, 95146,
   ...
   67817, 66609, 63340, 65512, 71594, 68836, 71399, 68010, 70177, 69702)
  / 100000)
m6.name <- 'sim6'  
...
m7 <- (c(
  100000, 99901, 98262, 96147, 96334, 95340, 95359, 96898, 96645, 95102, 94744,
   ...
   64540, 63160, 60706, 62340, 65974, 63724, 66402, 63517, 65418, 64702)
  / 100000)
m7.name <- 'sim7'  
...
(f1k.total <- f1kVal[length(f1kVal)]) # [  847,   5657] [2-param]
