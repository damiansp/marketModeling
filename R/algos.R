...
SPActual <- c(df$SPActual,
  ...
  4109,4121,4161,4018,3901,3750,3735,3790,3667,3675)
...
FidVal <- c(df$FidVal,
  ...
  186835,186732,190591,184515,188039) #
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
...
OHVal <-c(df$OHVal,    
   ...
   113572,113532,115921,112819,115593) #
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
m1 <- (c(
  100000, 99734, 97021, 92607, 93071, 91736, 92132, 94069, 93531, 90683, 90091,
   ...
   59158, 55092, 55119, 57372, 54004, 55788)
  / 100000)
#qp.start <- 92
m1.name <- 'sim1'
...
m2 <- (c(
  100000, 99519, 96234, 91473, 91946, 90337, 90702, 92795, 92113, 88699, 88134,
   ...
   57762, 54362, 54311, 56298, 53213, 54772)
  / 100000) 
m2.name <- 'sim2'
...
m3.raw <- c(
  100000, 99890, 96310, 91049, 91683, 89443, 89246, 91155, 90781, 88037, 87453, 
   ...
   66063, 62101, 62117, 64075, 61173, 62354)
m3 <- m3.raw / 100000
m3.name <- 'sim3'
...
m4.raw <- c(
  100000, 99289, 96063, 91314, 91644, 90074, 90327, 92360, 91830, 88380, 88025,
   ...
   59263, 55610, 55622, 57509, 54840, 56387)
m4 <- m4.raw / 100000
m4.name <- 'sim4'
...
m5 <- (c(
  100000, 99775, 96651, 91870, 92422, 91307, 92182, 94394, 93768, 89786, 89177,
   ...
   62262, 58388, 58333, 60657, 57482, 59196)
  / 100000)
m5.name <- 'sim5'  
...
m6 <- (c(
  100000,100012, 98204, 96007, 96400, 95686, 96971, 97980, 98605, 95083, 95146,
   ...
   67403, 63040, 62901, 65067, 61598, 63298)
  / 100000)
m6.name <- 'sim6'  
...
m7 <- (c(
  100000, 99901, 98262, 96147, 96334, 95340, 95359, 96898, 96645, 95102, 94744,
   ...
   63581, 59985, 60223, 62193, 59387, 60785)
  / 100000)
m7.name <- 'sim7'  
