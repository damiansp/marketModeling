...
SPActual <- c(df$SPActual,
    ...
    4278,4260,4204,4173,4262,4358,4412,4463)
...
FidIn <- c(df$FidIn,
  ...
  161785,161785,161785,161785,161785,161785,161785,161785,161785,161785,161785,
  161785,161785,161785,161785,161785,161785,161785,161785,161785) #
...
FidVal <- c(df$FidVal,
  ...
  214494,218804,217080,213226,207439,213845,227138,236682,250020) #
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
...
f1kIn <- c(df$f1kIn,
  ...
  0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,
    ...
    1306,  1306,  1306,  1306,  1306,  1306,  1306,  2023,  2023) #
f1kVal <- c(df$f1kVal,
  ...
  147813,146496,145095,141741,141741,141741,     1,     1,     1,     1,     1,
    ...
    1712,  1707,  1772,  1764,  1731,  1702,  1752,  2769,  2808) #
n <- length(f1kIn)
...
OHIn <- c(df$OHIn,
    ...
    55621, 55621, 55621, 55621, 57649, 57649, 58646, 58978, 58978) # 
OHVal <- c(df$OHVal, 
   ...
   101801,103713,103204,102112,103435,107191,117790,121618,127554) #
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
m1 <- (c(
  100000, 99734, 97021, 92607, 93071, 91736, 92132, 94069, 93531, 90683, 90091,
   ...
   77588, 77022, 80586, 79208, 75363, 71932, 74564, 79665, 82683, 85919)
  / 100000)
qp.start <- 92
m1.name <- 'sim1'
...
m2 <- (c(
  100000, 99519, 96234, 91473, 91946, 90337, 90702, 92795, 92113, 88699, 88134,
   ...
   74317, 73857, 77530, 76241, 72815, 69899, 72093, 76498, 78989, 81897)
  / 100000) 
m2.name <- 'sim2'
...
m3.raw <- c(
  100000, 99890, 96310, 91049, 91683, 89443, 89246, 91155, 90781, 88037, 87453, 
   ...
   80189, 79725, 82573, 81231, 77547, 74984, 77353, 81368, 84414, 87515)
m3 <- m3.raw / 100000
m3.name <- 'sim3'
...
m4.raw <- c(
  100000, 99289, 96063, 91314, 91644, 90074, 90327, 92360, 91830, 88380, 88025,
   ...
   75363, 74865, 78112, 77060, 73747, 70732, 73045, 77623, 80265, 83692)
m4 <- m4.raw / 100000
m4.name <- 'sim4'
...
m5 <- (c(
  100000, 99775, 96651, 91870, 92422, 91307, 92182, 94394, 93768, 89786, 89177,
   ...
   76314, 76131, 80113, 78878, 75174, 71810, 74769, 81328, 84511, 88583)
  / 100000)
m5.name <- 'sim5'  
...
m6 <- (c(
  100000,100012, 98204, 96007, 96400, 95686, 96971, 97980, 98605, 95083, 95146,
   ...
   82856, 81936, 87122, 85525, 81217, 78300, 81197, 86988, 90084, 95329)
  / 100000)
m6.name <- 'sim6'  
...
m7 <- (c(
  100000, 99901, 98262, 96147, 96334, 95340, 95359, 96898, 96645, 95102, 94744,
   ...
   83126, 82742, 86523, 85175, 81054, 77653, 80287, 85058, 88263, 91422)
  / 100000)
m7.name <- 'sim7'  
...
(fid.amt <- FidVal[length(FidVal)]) # [80533, 250020]
(oh.amt <- OHVal[length(OHVal)]) # [92495, 127554] # E*Trade
(f1k.total <- f1kVal[length(f1kVal)]) # [847, 2808] [2-param]
