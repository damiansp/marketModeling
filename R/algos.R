...
wlsh.osc <- c(
 ...
 -407,-419,-434,-456,-468,-467,-466,-472,-468,-467,-470)
...
SPActual <- c(df$SPActual,
    ...
    4471,4475,4380,4349,4305,4226,4289,4385,4374,4306,4387,4363,4329)
...
FidIn <- c(df$FidIn,
  ...
  161785,161785,161785,161785,161785,161785,161785,161785,161785,161785) #
...
FidVal <- c(df$FidVal,
  ...
  220354,218267,212341,223870,228841,232960,230158,230738,225290,220206) #
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
...
f1kIn <- c(df$f1kIn,
  ...
  0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,
     620,   620,   620,   620,   620,   620,   620,   620,   620,  1306) #
f1kVal <- c(df$f1kVal,
  ...
  147813,146496,145095,141741,141741,141741,     1,     1,     1,     1,     1,
     ...
     863,   855,   847,   824,   854,   867,   867,   852,   869,  1818) #
n <- length(f1kIn)
...
OHIn <- c(df$OHIn,
    ...
    51650, 51650, 51650, 52046, 52789, 53816, 53816, 55281, 55281, 55281) #
OHVal <- c(df$OHVal, 
   102557,100422, 97844, 98121, 97375, 97932, 98939, 99051, 97002, 96791, 95665,
    ...
    95617, 94996, 92698, 98555,101932,105235,104528,107470,105753,103912) #
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
(fid.amt <- FidVal[length(FidVal)]) # [80533, 232960]
(oh.amt <- OHVal[length(OHVal)]) # [92495, 107470] # E*Trade
(f1k.total <- f1kVal[length(f1kVal)]) # [847, 1818] [2-param]
