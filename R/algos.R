...
wlsh.osc <- c(
  ...
  -212,-220,-223,-226,-233,-239,-243,-239,-231,-223,-211,-199,-186,-175,-162,
  -150,-143,-131,-119,-106, -93, -77, -61, -45
  )
...
SPActual <- c(df$SPActual,
    ...
    4520,  4536,  4550,  4545,  4566,  4575,  4552,  4596,  4605,  4614,  4631,
    4661,  4680,  4698)
...
FidVal <- c(df$FidVal,
  ...
  110947,110808,112199,110923,111854,111246,109690,111697,112290,113796,112834,
  113017,111361,110301) #
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
...
f1kIn <- c(df$f1kIn,
   ...
   95368, 95368, 95368, 95368, 95368, 95368, 95368, 95368, 96052, 96052, 96052,
   96052, 96052, 96052) #
f1kVal <- c(df$f1kVal,
  ...
  148732,149213,149555,149955,149573,150273,150129,149100,151585,151598,152587,
  152869,153871,155238) #
payment <- 1018.67
n <- length(f1kIn)
...
OHIn <- c(df$OHIn,
   ...
   44960, 44960, 44960, 44960, 44960, 44960, 44960, 44960, 45377, 45377, 45377,
   45377, 45377, 45377) #
OHVal <- c(df$OHVal, 
  ...
  104997,105043,106099,105779,106101,105414,104047,104827,106521,107566,106803,
  106839,108149,107300) #
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
m1 <- (c(
  ...
  117336,110919,108642,108221,101922,104631,106850,105531, 98774, 94278, 93620,
   ...
   98262, 99509, 99389,100557, 99453,100382, 99856, 98593, 99675, 99787,100773,
  100533,101354,101658,100817)
  / 100000)
qp.start <- 92
m1.name <- 'BuyNHold'
...
m2 <- (c(
  ...
  100681,102309,102611,103440,102272,102639,102370,100837,101934,101950,102635,
  102034,101700,102375,101728)
  / 100000) 
...
m3.raw <- c(
  ...
  100929,102629,103292,104157,101889,102615,101207, 99183,100720,100335,102107,
  100614,100991,100934, 98820)
m3 <- m3.raw / 100000
m3.name <- 'CashInLow'
#len <- length(m3)
...
m4.raw <- c(
  ...
  100388,101091,101426,102517,101401,102355,101543, 99925,101818,103229,103785,
  103936,103389,104691,104348)
m4 <- m4.raw / 100000
m4.name <- 'CashInHigh'
len <- length(m4)
...
m5 <- (c(
  ...
  103753,105467,106218,107606,106166,106985,105489,103592,104356,104338,105491,
  104316,104586,105834,103677)
  / 100000)
m5.name <- '30/70'  
...
m6 <- (c(
  ...
  105194,106771,106583,107495,106916,106984,107621,105526,106327,108379,109036,
  107382,106360,108047,107108)
  / 100000)
m6.name <- 'rsi each'  
...
m7 <- (c(
  ...
  100773,103224,103060,103862,103035,103035,102251,100294,102059,102480,104459,
  102118,101627,101172,100250)
  / 100000)
m7.name <- 'rsi overall'  
...
# Mon
wil3 <- c(1, 1, 1)
nas3 <- c(0.9296,  1, 0.9898)

# Tue
wil5 <- c(0.9711, 1, 0.9999)
nas5 <- c(1, 1, 1)
...
# Thu
wil20 <- c(0.0001, 0.9999, 0.9968)
nas20 <- c(1, 1, 0.9988)

# Fri
nya35 <- c(1, 1, 0)
nas35 <- c(0.9989, 0.9915, 1)

# Sat
nya50 <- c(1, 1, 1)
nas50 <- c(0.9908, 0.9993, 0.9900)
...
# For 401(k): # Next:2 dec (≥10%)
(f1k.total <- f1kVal[length(f1kVal)]) # [118811, 155238] [2-param]
(f1k.pct.in <- w %*% c(
