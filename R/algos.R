...
sp.actual <- c(
  df$sp.actual,   5827,  5836,  5843,  5950, 5937,  5997,  6049, 6086,  6119,
    6101,  6012,  6068,  6039,  6071,  6041, 5995,  6038,  6061, 6084)
...
fid.val <- c(
  ...
  264323,265178,266072,266541,268886,266966,266306,266836,267983,268732) #

f1k.in <- c(
  df$f1k.in,     52664, 52664, 52664, 52664, 53385, 53385, 53385, 53385, 53385,
   54088, 54088, 54088, 54088, 54088, 54088, 54088, 54088, 54088, 54794) # (1125.13)
f1k.val <- c(
  df$f1k.val,    82324, 82169, 82228, 82282, 83645, 83548, 84187, 84966, 85417,
   86877, 86687, 85482, 86377, 86028, 86597, 86145, 85505, 86165, 87586) #

self.mng.in <- c(
  df$self.mng.in,13857, 13857, 13857, 13857, 13857, 13857, 13857, 13857, 13857,
   13857, 13857, 13857, 13857, 13857, 13857, 13857, 13857, 13857, 13857)  #
self.mng.val <- c(
  df$self.mng.val, 
                 17468, 17480, 17547, 17681, 17776, 17783, 17934, 17906, 17923,
   17937, 17950, 17966, 17951, 18100, 17981, 17925, 18049, 18151, 18153)  #

et.in <- c(
  df$et.in,      92607, 92997, 93120, 93120, 93120, 93120, 93120, 93120, 93120,
   93120, 93120, 93120, 93120, 93330, 93330, 93330, 93330, 93330, 93330) # 
et.val <-c(
  df$et.val,    211043,212988,214224,214870,216300,216008,216174,216336,216595,
  215513,217232,218083,218359,220065,218102,217139,217036,217815,218562) #

sim1 <- (c(
         100000,100000,100181,100920,102382,102281,101603,103160,104907,104306,
  104823,106238,107565,107041,109842,109042,107404,107212,110907,112705)
  / 100000)
sim1.res <- 57636
...
sim2 <- (c(
  ...
  106168,106180,109058,108848,110943,109775,107708,107689,109993,111953)
  / 100000)
sim2.res <- 57395
...
sim3 <- (c(
  ...
  104101,105513,107951,107052,109536,108744,106686,106743,110295,111389)
  / 100000)
sim3.res <- 56725
...
sim4 <- (c(
  ...
  103325,105111,106746,106348,109165,108556,106282,106247,109595,111455)
  / 100000)
sim4.res <- 57129
...
sim5 <- (c(
  ...
  105501,106753,108745,108704,110745,109261,106735,106840,109390,110814)
  / 100000)
sim5.res <- 56665
...
extr <- (c(
   ...
   99729, 99185,101753,100747,103563,101707, 98628, 98969,103628,106584)
  / 100000)
extr.res <- 5668
