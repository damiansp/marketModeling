...
wlsh.osc <- c(
 ...
 -325,-357,-375,-381,-381,-379,-388,-392,-397,-396,-386,-386,-394)

SPActual <- c(df$SPActual,
    4797,4794,4701,4696,4677,4670,4713,4726,4659,4663,4577,4533,4483,4398,4410,
    4356,4350,4327,4432,4516,4547,4589,4477,4501,4484,4522,4587,4504,4419)
...
FidVal <- c(df$FidVal,
  ...
  221810,225872,225565,227194,232232,231501,226454) #
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
...
f1kIn <- c(df$f1kIn,
  ...
  0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935) # 
f1kVal <- c(df$f1kVal,
...
OHIn <- c(df$OHIn,
    ...
    49864, 49864, 49864, 49864, 49864, 49864, 49864) #
OHVal <- c(df$OHVal, 
   102557,100422, 97844, 98121, 97375, 97932, 98939, 99051, 97002, 96791, 95665,
    ...
    94494, 97379, 97364, 98090,100648,100016, 98296) #
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
m1 <- (c(
  100000, 99734, 97021, 92607, 93071, 91736, 92132, 94069, 93531, 90683, 90091,
   ...
   86042, 83068, 85330, 85522, 87001, 90263, 89454, 87144)
  / 100000)
qp.start <- 92
m1.name <- 'sim1'
...
m2 <- (c(
  100000, 99519, 96234, 91473, 91946, 90337, 90702, 92795, 92113, 88699, 88134,
   ...
   84916, 81811, 83915, 84091, 85475, 88480, 87430, 84757)
  / 100000) 
m2.name <- 'sim2'
...
m3.raw <- c(
  100000, 99890, 96310, 91049, 91683, 89443, 89246, 91155, 90781, 88037, 87453, 
   ...
   84065, 80991, 82349, 81679, 83145, 87233, 85828, 83761)
m3 <- m3.raw / 100000
m3.name <- 'sim3'
...
m4.raw <- c(
  100000, 99289, 96063, 91314, 91644, 90074, 90327, 92360, 91830, 88380, 88025,
   ...
   84282, 81703, 83586, 83495, 84327, 87216, 86221, 83907)
m4 <- m4.raw / 100000
m4.name <- 'sim4'
...
m5 <- (c(
  100000, 99775, 96651, 91870, 92422, 91307, 92182, 94394, 93768, 89786, 89177,
   ...
   85575, 82009, 84571, 84519, 85750, 89164, 87765, 85391)
  / 100000)
m5.name <- 'sim5'  
...
m6 <- (c(
  100000,100012, 98204, 96007, 96400, 95686, 96971, 97980, 98605, 95083, 95146,
   ...
   94775, 91892, 93896, 93335, 94248, 97118, 96630, 94595)
  / 100000)
m6.name <- 'sim6'  
...
m7 <- (c(
  100000, 99901, 98262, 96147, 96334, 95340, 95359, 96898, 96645, 95102, 94744,
   ...
   91146, 88692, 89998, 90197, 90946, 93189, 92856, 91261)
  / 100000)
m7.name <- 'sim7'  
...
cash.out.fid <- 114061 # Update ONLY when new threshold crossed

(cash.out.et <- CASH_OUT * OHVal[length(OHVal)])
cash.out.et <- 49200

(cash.out.f1k <- CASH_OUT * f1kVal[length(f1kVal)])
cash.out.f1k <- 0

crypto.frac <- 0.003
(target.crypo.amt <- crypto.frac * totval[length(totval)]) # in: 1100

(fid.amt <- FidVal[length(FidVal)]) # [80533, 232595]
(oh.amt <- OHVal[length(OHVal)]) # [92495, 100868]
...
#nya40 <- c(1, 1)
#nas40 <- c(0.9987, 0.0149)

#wil30 <- c(1, 1)
#nas30 <- c(0.0103, 0.9999)

#wil20 <- c(0.9905, 0.9999)#
#nas20 <- c(0.1642, 0.9999)

#wil10 <- c(0.9896, 1)
#nas10 <- c(1, 0.9321)

#wil5 <- c(1, 0.5148)
#nas5 <- c(0.7353, 0.0712)
...
#(fid.amt <- FidVal[length(FidVal)]) # [80533, 229970]
#(fid.pct.in <- w %*% c(
