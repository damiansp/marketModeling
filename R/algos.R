...
SPActual <- c(df$SPActual,
    ...
    4523,  4524,  4537,  4535,  4514,  4493,  4459,  4469,  4443)
...
FidVal <- c(df$FidVal,
  ...
  112180,113632,113336,113576,111785,112347,111865,111055,110272)
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
...
f1kIn <- c(df$f1kIn,
   ...
   93285, 93285, 93285, 93285, 93285, 93285, 93285, 93285, 93285) #
f1kVal <- c(df$f1kVal,
  ...
  146142,146142,146551,146551,145349,145349,145013,144265,143827) #
payment <- 1018.67
n <- length(f1kIn)
...
OHIn <- c(df$OHIn,
   ...
   42552, 42874, 42874, 42874, 42874, 42874, 42874, 42874, 42874) #
OHVal <- c(df$OHVal, 
  ...
  101622,101246,103242,102694,103325,102648,102961,100355,101452, 87037, 86693,
   ...
   96806, 98546, 98505, 99118, 97763, 98736, 98238, 97256, 97174) # 
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
m1 <- (c(
  ...
  117336,110919,108642,108221,101922,104631,106850,105531, 98774, 94278, 93620,
   ...
   98093, 98299, 99140, 99848,101014, 99445, 99682, 99038, 97995, 97643)
  / 100000)
qp.start <- 92
m1.name <- 'BuyNHold'
...
m2 <- (c(
  ...
  106966,106520,108051,107668,108095,105836,106558,105886,104876,103839)
  / 100000) 
...
m3.raw <- c(
  ...
  105487,105340,107042,107443,107928,105601,105803,105309,104110,103129)
m3 <- m3.raw / 100000
m3.name <- 'CashInLow'
#len <- length(m3)
...
m4.raw <- c(
  ...
  101283,101490,102260,102439,104144,101691,101668,100958,100068,100137)
m4 <- m4.raw / 100000
m4.name <- 'CashInHigh'
len <- length(m4)
...
m5 <- (c(
  ...
  105363,105457,106680,107205,108625,106709,107124,106896,105661,104937)
  / 100000)
m5.name <- '30/70'  
...
m6 <- (c(
  ...
  108285,109107,109680,110727,111329,110030,110023,109432,108931,107900)
  / 100000)
m6.name <- 'rsi each'  
...
m7 <- (c(
  ...
  100999,100935,101636,101615,102230,100958,101478,101015,100369, 99948)
  / 100000)
m7.name <- 'rsi overall'  
...
crypto.frac <- 0.003
(target.crypo.amt <- crypto.frac * totval[length(totval)]) # in: 1000
...
# Mon
wil3 <- c(1, 1 , 0.9999)
nas3 <- c(1, 0.9999, 0.9989)

# Tue
wil5 <- c(0.9921, 1, 0.9933)
nas5 <- c(0.9999, 0.9594, 1)

# Wed    
wil10 <- c(1, 0.9999, 0.9502)
nas10 <- c(1, 1, 0.0001)
...
# Fri
nya35 <- c(1, 0.9848, 0.0107)
nas35 <- c(0.9989, 1, 1)

# Sat
nya50 <- c(1, 1, 1)
nas50 <- c(0.9910, 1, 0.9999)
...
# For 401(k): # Next: 01 Sep (≥10%)
(f1k.total <- f1kVal[length(f1kVal)]) # [118811, 146551] [2-param]
