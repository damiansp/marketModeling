#---------#---------#---------#---------#---------#---------#---------#---------
load('~/marketModeling/data/myHistoric.RData')
head(df)
tail(df)
...
FidIn <- c(df$FidIn,
   ...
   57134, 57134, 57134, 57134, 57134, 57134, 57134, 57134, 57134, 57134, 57134,
   57134, 57134, 57134, 57134, 57134, 57134, 57134, 57134, 57134, 57134, 57134,
   57134, 57134, 57134, 57134, 57134, 57134)
...
FidVal <- c(df$FidVal,
  ...
  110745,109427,109103,108323,107464,108693)
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
...
f1kIn <- c(df$f1kIn,
   ...
   91903, 92593, 92593, 92593, 92593, 92593) #
f1kVal <- c(df$f1kVal,
  ...
  142287,142926,142494,140990,139899,139125) #
payment <- 1018.67
n <- length(f1kIn)
...
OHIn <- c(df$OHIn,
   ...
   42552, 42552, 42552, 42552, 42552, 42552) #
OHVal <- c(df$OHVal, 
  ...
  101622,101246,103242,102694,103325,102648,102961,100355,101452, 87037, 86693,
   ...
   95372, 94478, 94345, 93929, 93370, 94240) # 
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
m1 <- (c(
  ...
  117336,110919,108642,108221,101922,104631,106850,105531, 98774, 94278, 93620,
   ...
   96611, 96350, 95163, 94898, 94640, 93577, 94440)
  / 100000)
qp.start <- 92
m1.name <- 'BuyNHold'
...
m2 <- (c(
  ...
  105172,104471,102917,102628,101697,100871,102314)
  / 100000) 
...
m3.raw <- c(
  ...
  103736,102384,100637,100474, 99659, 98490,100130)
m3 <- m3.raw / 100000
m3.name <- 'CashInLow'
#len <- length(m3)
...
m4.raw <- c(
  ...
  102095,101168, 99047, 99236, 99148,100697,101348, 99578, 99962, 98588, 98143,
   98914, 98308, 97095, 96945, 96388, 96266, 97333)
m4 <- m4.raw / 100000
m4.name <- 'CashInHigh'
len <- length(m4)
...
m5 <- (c(
  ...
  103018,102154,100505,100530, 99789, 99008,100488)
  / 100000)
m5.name <- '30/70'  
...
m6 <- (c(
  ...
  104387,105285,103868,103537,103536,102871,103996)
  / 100000)
m6.name <- 'rsi each'  
...
m7 <- (c(
  ...
  100305, 99918, 98989, 98820, 98551, 98218, 98856)
  / 100000)
m7.name <- 'rsi overall'  
...
crypto.frac <- 0.003
(target.crypo.amt <- crypto.frac * totval[length(totval)])
...
# Tue
wil5 <- c(0.9970, 1, 0.9947)
nas5 <- c(1, 1, 0.9420)

# Wed    
wil10 <- c(1, 0.9999, 0.9933)
nas10 <- c(1, 1, 0.9901)

# Thu
wil20 <- c(0.3405, 0.0003, 0)
nas20 <- c(1, 1, 0.9959)

# Fri
nya35 <- c(1, 0.9871, 0.9909)
...
(fid.res <- fid.amt - fid.amt.in) # 375
...
(f1k.total <- f1kVal[length(f1kVal)]) # [118811, 142494] [2-param]
