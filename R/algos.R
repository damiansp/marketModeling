...
wlsh.osc <- c(
 ...
 -181,-173,-164,-157,-157,-158,-152,-152)
...
SPActual <- c(df$SPActual,
    ...
    4602,4530,4546,4583,4525,4481,4500,4488,4413,4397,4447,4393)
...
FidIn <- c(df$FidIn,
  ...
  161785,161785,161785,161785,161785,161785,161785,161785,161785,161785,161785,
  161785,161785,161785,161785,161785,161785,161785,161785,161785,161785,161785,
  161785,161785,161785,161785,161785,161785) #
...
FidVal <- c(df$FidVal,
  ...
  237355,231025,228022,224991,231757,223658) #
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
...
OHIn <- c(df$OHIn,
    ...
    59760, 59760, 59760, 59760, 59760, 59760) # 1669
OHVal <- c(df$OHVal, 
   ...
   126916,124159,122811,121324,125142,121775) #
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
m1 <- (c(
  100000, 99734, 97021, 92607, 93071, 91736, 92132, 94069, 93531, 90683, 90091,
   ...
   84010, 83687, 81901, 81058, 80459, 82912, 80389)
  / 100000)
#qp.start <- 92
m1.name <- 'sim1'
...
m2 <- (c(
  100000, 99519, 96234, 91473, 91946, 90337, 90702, 92795, 92113, 88699, 88134,
   ...
   79775, 79483, 77784, 76954, 76282, 78337, 76131)
  / 100000) 
m2.name <- 'sim2'
...
m3.raw <- c(
  100000, 99890, 96310, 91049, 91683, 89443, 89246, 91155, 90781, 88037, 87453, 
   ...
   86036, 85895, 83995, 82998, 82196, 84528, 82200)
m3 <- m3.raw / 100000
m3.name <- 'sim3'
...
m4.raw <- c(
  100000, 99289, 96063, 91314, 91644, 90074, 90327, 92360, 91830, 88380, 88025,
   ...
   81387, 81126, 79584, 78809, 78162, 80299, 78306)
m4 <- m4.raw / 100000
m4.name <- 'sim4'
...
m5 <- (c(
  100000, 99775, 96651, 91870, 92422, 91307, 92182, 94394, 93768, 89786, 89177,
   ...
   86336, 85805, 84045, 83406, 82627, 85007, 82435)
  / 100000)
m5.name <- 'sim5'  
...
m6 <- (c(
  100000,100012, 98204, 96007, 96400, 95686, 96971, 97980, 98605, 95083, 95146,
   ...
   93601, 93350, 91191, 90400, 89442, 92042, 89235)
  / 100000)
m6.name <- 'sim6'  
...
m7 <- (c(
  100000, 99901, 98262, 96147, 96334, 95340, 95359, 96898, 96645, 95102, 94744,
   ...
   89070, 89063, 87319, 86410, 85787, 88088, 85587)
  / 100000)
m7.name <- 'sim7'  
...
(f1k.total <- f1kVal[length(f1kVal)]) # [  847,   3833] [2-param]
