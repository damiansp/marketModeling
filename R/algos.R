...
sp.actual <- c(
  df$sp.actual,     4743,  4705,  4689,  4697,  4764,  4757,  4783,  4780,  4784,
      ...
      5188,  5214,  5223,  5221,  5247,  5308,  5297,  5303,  5308,  5321)
...
fid.val <- c(
  df$fid.val,     226255,222123,222288,222370,226164,226085,226787,226918,226111,
    ...
    227183,229108,228380,230280,232608,236093,234594,235648,239119,234184) #
...
f1k.in <- c(
  df$f1k.in,       36528, 36528, 36528, 36528, 36528, 36528, 36528, 36528, 36528,
     ...
     42720, 42720, 42720, 42720, 42720, 42720, 43468, 43468, 43468, 43468) # (1062.63)
f1k.val <- c(
  df$f1k.val,      48986, 48283, 47658, 47523, 47500, 48441, 48481, 48834, 48889,
     ...
     59960, 59764, 60095, 60165, 60311, 60699, 62436, 62301, 62358, 62481) #
...
self.mng.val <- c(
  df$self.mng.val, 16169, 15852, 15887, 15861, 16152, 16065, 16106, 16048, 15949,
     ...
     15701, 15785, 15790, 15955, 16086, 16216, 16193, 16196, 16218, 16085)  #
...
et.in <- c(
  df$et.in,        86278, 86278, 86278, 86278, 86278, 86278, 86278, 86278, 86588,
     ...
     87125, 87125, 87125, 87125, 87394, 87394, 87394, 87394, 87394, 87394) #
et.val <-c(
  df$et.val,      182169,178447,178828,178839,181753,181913,181774,181776,181803,
    ...
    176517,178393,177761,178973,181929,184826,183913,184471,183928,181289) #
...
sim1 <- (c(
    ...
    100247,100114,100122,100199,100338,101076,100894,100912,101130,100077)
  / 100000)
sim1.res <- 6981
...
sim2 <- (c(
    ...
    101111, 98406, 98202, 98586, 99017,100311, 99984,100143,100661, 98356)
  / 100000)
sim2.res <- 6777
...
sim3 <- (c(
    ...
    100339, 98983, 99250, 99136, 99658,101156,101057,101026,101527,100109)
  / 100000)
sim3.res <- 3384
