...
SPActual <- c(df$SPActual,
    ...
    4471,4475,4380,4349,4305,4226,4289,4385)
...
FidIn <- c(df$FidIn,
  ...
  161785,161785,161785,161785,161785) #
...
FidVal <- c(df$FidVal,
  ...
  220354,218267,212341,223870,228841) #
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
...
OHIn <- c(df$OHIn,
    ...
    51650, 51650, 51650, 52046, 52789) # 1983
OHVal <- c(df$OHVal, 
   102557,100422, 97844, 98121, 97375, 97932, 98939, 99051, 97002, 96791, 95665,
    ...
    95617, 94996, 92698, 98555,101932) #
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
m1 <- (c(
  100000, 99734, 97021, 92607, 93071, 91736, 92132, 94069, 93531, 90683, 90091,
   ...
   85501, 82663, 81832, 79154, 83928, 85585)
  / 100000)
qp.start <- 92
m1.name <- 'sim1'
...
m2 <- (c(
  100000, 99519, 96234, 91473, 91946, 90337, 90702, 92795, 92113, 88699, 88134,
   ...
   82882, 80170, 79277, 76822, 81232, 82869)
  / 100000) 
m2.name <- 'sim2'
...
m3.raw <- c(
  100000, 99890, 96310, 91049, 91683, 89443, 89246, 91155, 90781, 88037, 87453, 
   ...
   82981, 80101, 79476, 77510, 84759, 86351)
m3 <- m3.raw / 100000
m3.name <- 'sim3'
...
m4.raw <- c(
  100000, 99289, 96063, 91314, 91644, 90074, 90327, 92360, 91830, 88380, 88025,
   ...
   82848, 80632, 79966, 78132, 82004, 83071)
m4 <- m4.raw / 100000
m4.name <- 'sim4'
...
m5 <- (c(
  100000, 99775, 96651, 91870, 92422, 91307, 92182, 94394, 93768, 89786, 89177,
   ...
   83335, 80597, 79997, 77851, 82453, 84232)
  / 100000)
m5.name <- 'sim5'  
...
m6 <- (c(
  100000,100012, 98204, 96007, 96400, 95686, 96971, 97980, 98605, 95083, 95146,
   ...
   92644, 89662, 89136, 86860, 91581, 94435)
  / 100000)
m6.name <- 'sim6'  
...
m7 <- (c(
  100000, 99901, 98262, 96147, 96334, 95340, 95359, 96898, 96645, 95102, 94744,
   ...
   90538, 88038, 87669, 86034, 90318, 91664)
  / 100000)
m7.name <- 'sim7'  
...
(oh.amt <- OHVal[length(OHVal)]) # [92495, 101932] # E*Trade
(f1k.total <- f1kVal[length(f1kVal)]) # [847, 855] [2-param]
