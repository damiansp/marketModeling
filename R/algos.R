...
SPActual <- c(df$SPActual,
  ...
  4305,4274,4284,4228,4138,4129,4141,4199,4058)
...
FidVal <- c(df$FidVal,
  ...
  211579,210857,213022,217666,210247) #
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
...
f1kIn <- c(df$f1kIn,
  ...
  0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,
   ...
   10636, 10636, 10636, 10636, 10636) # 961.47
f1kVal <- c(df$f1kVal,
...
OHVal <-c(df$OHVal,    
   ...
   137877,137117,138688,142338,137044) #
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
m1 <- (c(
  100000, 99734, 97021, 92607, 93071, 91736, 92132, 94069, 93531, 90683, 90091,
   ...
   62844, 61039, 60813, 61646, 63418, 60836)
  / 100000)
qp.start <- 92
m1.name <- 'sim1'
...
m2 <- (c(
  100000, 99519, 96234, 91473, 91946, 90337, 90702, 92795, 92113, 88699, 88134,
   ...
   61651, 60077, 59833, 60579, 62158, 59706)
  / 100000) 
m2.name <- 'sim2'
...
m3.raw <- c(
  100000, 99890, 96310, 91049, 91683, 89443, 89246, 91155, 90781, 88037, 87453, 
   ...
   70194, 68294, 68200, 68873, 70639, 67950)
m3 <- m3.raw / 100000
m3.name <- 'sim3'
...
m4.raw <- c(
  100000, 99289, 96063, 91314, 91644, 90074, 90327, 92360, 91830, 88380, 88025,
   ...
   64292, 62723, 62425, 63160, 64573, 61977)
m4 <- m4.raw / 100000
m4.name <- 'sim4'
...
m5 <- (c(
  100000, 99775, 96651, 91870, 92422, 91307, 92182, 94394, 93768, 89786, 89177,
   ...
   69521, 67782, 67359, 68203, 69940, 67318)
  / 100000)
m5.name <- 'sim5'  
...
m6 <- (c(
  100000,100012, 98204, 96007, 96400, 95686, 96971, 97980, 98605, 95083, 95146,
   ...
   73910, 71999, 71810, 72501, 74313, 71376)
  / 100000)
m6.name <- 'sim6'  
...
m7 <- (c(
  100000, 99901, 98262, 96147, 96334, 95340, 95359, 96898, 96645, 95102, 94744,
   ...
   70599, 68955, 68868, 69496, 71130, 68581)
  / 100000)
m7.name <- 'sim7'  
...
(self.mng.amt <- self.mng.val[length(self.mng.val)]) # [ 4920,   5460]
