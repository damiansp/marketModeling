...
# damiansp1976
wlsh.osc <- c(
  -77, -73, -80, -88, -97,-106,-110,-112,-122,-132)

SPActual <- c(df$SPActual,
    4797,4794,4701,4696,4677,4670,4713,4726,4659,4663)
...
FidVal <- c(df$FidVal,
   94096, 90621, 86085, 86306, 84787, 85416, 87205, 87106, 83943,83649) #
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
Fidd <- dailyChange(Fid)

f1kIn <- c(df$f1kIn,
   98297, 98297, 98297, 98297, 98297, 98297, 98297, 98297, 98297, 98297) # 
f1kVal <- c(df$f1kVal,
  157563,158404,157348,153323,153323,151838,151838,153394,153734,150997) #
n <- length(f1kIn)
...
OHVal <- c(df$OHVal, 
   102557,100422, 97844, 98121, 97375, 97932, 98939, 99051, 97002, 96791) #
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
# Thu
wil20 <- c(1, 1, 0.9999)
nas20 <- c(1, 0.1989, 0.9999)

# Fri
nya35 <- c(1, 1, 1)
nas35 <- c(0.9989, 0.1114, 1)
...
(fid.amt <- FidVal[length(FidVal)]) # [83943, 90621]
(fid.pct.in <- w %*% c(
...
(oh.amt <- OHVal[length(OHVal)]) # [97002, 100422]
(oh.pct.in <- w %*% c(
...
(f1k.total <- f1kVal[length(f1kVal)]) # [151838, 158404] [2-param]
(f1k.pct.in <- w %*% c(
