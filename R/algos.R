...
sp.actual <- c(
  df$sp.actual,    3824,  3853,  3808,  3895,  3892,  3919,  3970,  3987,  3999,  
     ...
     4472,  4510,  4505,  4523,  4555,  4566,  4535,  4536)
...
fid.in <- c(
  df$fid.in,     161785,161785,161785,161785,161785,161785,161785,162813,162813,
   ...
   167137,167137,167137,167137,167137,167137,167137,167137) #
fid.val <- c(
  df$fid.val,    172350,175073,172978,176119,179159,182181,185241,188878,190980,
   ...
   216527,217844,217131,217574,216888,216996,216093,216845) #
...
f1k.in <- c(
  df$f1k.in,      18031, 18031, 18031, 18031, 18031, 18031, 18031, 18031, 18031,
    ...
    29950, 29950, 29950, 29950, 29950, 29950, 30856, 30856) # (1133.49)
f1k.val <- c(
  df$f1k.val,     17787, 17715, 17856, 17766, 18063, 18219, 18434, 18750, 18928,
    ...
    36606, 36810, 37202, 37174, 37367, 37469, 38615, 38113) #
...
self.mng.val <- c(
  df$self.mng.val, 9189,  9333,  9243,  9351,  9514,  9655,  9818,  9938, 10033,
    ...
    15975, 16089, 16019, 15937, 15968, 15953, 15865, 15905)
...
et.in <- c(
  df$et.in,       76890, 78443, 78443, 78443, 78443, 78443, 78443, 78443, 78443,
    ...
    79400, 79695, 79695, 79695, 79695, 79695, 79695, 79695) #
et.val <-c(
  df$et.val,     123916,128480,126726,128449,130010,131945,134582,135693,137333,
   ...
   151975,152951,152709,153063,152990,152977,152493,153247) #
...
rsi <- (c(
   ...
   114685,116695,115992,117456,116826,116756,114849,115827)
  / 100000)
...
adl <- (c(
   ...
   103583,105528,105047,106848,106958,106872,105139,107280)
  / 100000)
...
(et.amt <- et.val[length(et.val)])                   # [130010 153247]
(f1k.total <- f1k.val[length(f1k.val)])              # [ 18063  38113] 
