...
sp.actual <- c(
  df$sp.actual,     4743,  4705,  4689,  4697,  4764,  4757,  4783,  4780,  4784,
      4766,  4739,  4781,  4840,  4850,  4865,  4869,  4894,  4891)

fid.in <- c(
  df$fid.in,      167137,167137,167137,167137,167137,167137,167137,167137,167137,
    167137,167137,167137,167137,167137,167137,167137,167137,167137) #
fid.val <- c(
  df$fid.val,     226255,222123,222288,222370,226164,226085,226787,226918,226111,
    224232,223153,225424,226784,228372,228427,227347,228082,228542) #

f1k.in <- c(
  df$f1k.in,       36528, 36528, 36528, 36528, 36528, 36528, 36528, 36528, 36528,
     36528, 36528, 36528, 37334, 37334, 37334, 37334, 37334, 37334) # (1062.63)
f1k.val <- c(
  df$f1k.val,      48986, 48283, 47658, 47523, 47500, 48441, 48481, 48834, 48889,
     48787, 48421, 48149, 49621, 50044, 50275, 50379, 50236, 50386) #
...
self.mng.val <- c(
  df$self.mng.val, 16169, 15852, 15887, 15861, 16152, 16065, 16106, 16048, 15949,
     15728, 15640, 15791, 15827, 15929, 15951, 15813, 15829, 15851)  #

et.in <- c(
  df$et.in,        86278, 86278, 86278, 86278, 86278, 86278, 86278, 86278, 86588,
     86588, 86588, 86588, 86588, 86588, 86588, 86588, 86588, 86588) #
et.val <-c(
  df$et.val,      182169,178447,178828,178839,181753,181913,181774,181776,181803,
    179567,178132,180171,181513,182917,183106,181365,181575,181724) #

sim1 <- (c(
           100000,100023, 99980, 99994, 99987,100290,100240,100401,100414,100410,
    100359,100355,100424,100650,100768,101019,100920,100574,100565)
  / 100000)
sim1.name <- 'adelaide'

sim2 <- (c(
           100000,100005, 99939, 99944, 99916,100348,100284,100473,100482,100456,
    100392,100352,100383,100598,100832,100945,100791,100552,100503)
  / 100000)
sim2.name <- 'aei'

sim3 <- (c(
           100000, 99994, 99910, 99915, 99882,100396,100317,100507,100498,100453,
    100374,100335,100273,100497,100651,100707,100672,100509,100488)
  / 100000)
sim3.name <- 'simsims'
