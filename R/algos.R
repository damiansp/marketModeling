...
sp.actual <- c(
  df$sp.actual,     4743,  4705,  4689,  4697,  4764,  4757,  4783,  4780,  4784,
      ...
      5079,  5105,  5157,  5124,  5118,  5175,  5165,  5150,  5117)
...
fid.in <- c(
  df$fid.in,      167137,167137,167137,167137,167137,167137,167137,167137,167137,
    ...
    168659,168659,168659,168659,168659,168659,168659,169043,169043) #
fid.val <- c(
  df$fid.val,     226255,222123,222288,222370,226164,226085,226787,226918,226111,
    ...
    230947,232421,234109,233753,234290,234691,234813,233187,232082) #
...
f1k.in <- c(
  df$f1k.in,       36528, 36528, 36528, 36528, 36528, 36528, 36528, 36528, 36528,
     ...
     38878, 38878, 39644, 39644, 39644, 39644, 39644, 39644, 39644) # (1062.63)
f1k.val <- c(
  df$f1k.val,      48986, 48283, 47658, 47523, 47500, 48441, 48481, 48834, 48889,
     ...
     54507, 53928, 55347, 55884, 55505, 55488, 55855, 55779, 55440) #
...
self.mng.val <- c(
  df$self.mng.val, 16169, 15852, 15887, 15861, 16152, 16065, 16106, 16048, 15949,
     ...
     15835, 15960, 16084, 16004, 16053, 16062, 16076, 15888, 15863)  #
...
et.val <-c(
  df$et.val,      182169,178447,178828,178839,181753,181913,181774,181776,181803,
    ...
    177915,179389,180992,181081,181587,181473,182144,180261,179555) #
...
sim1 <- (c(
    ...
    101215,101635,102451,102399,103217,103459,103339,102972,102082)
  / 100000)
sim1.name <- 'adelaide'
...
sim2 <- (c(
    ...
    100497,100607,102200,102401,103780,104232,103814,102775,100948)
  / 100000)
sim2.name <- 'aei'
...
sim3 <- (c(
    ...
    104068,103945,105697,105423,106596,106857,106916,105856,104180)
  / 100000)
sim3.name <- 'simsims'
