...
SPActual <- c(df$SPActual,
    4797,4794,4701,4696,4677)
SP <- SPActual / SPActual[last]
SPd <- dailyChange(SP)

FidIn <- c(df$FidIn,
   57134, 57134, 57134, 57134, 57134)
  
FidVal <- c(df$FidVal,
   94096, 90621, 86085, 86306, 84787) #
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
Fidd <- dailyChange(Fid)

f1kIn <- c(df$f1kIn,
   98297, 98297, 98297, 98297, 98297) # 
f1kVal <- c(df$f1kVal,
  157563,158404,157348,153323,153323) #
n <- length(f1kIn)
...
OHIn <- c(df$OHIn,
   49269, 49269, 49269, 49269, 49269) #
OHVal <- c(df$OHVal, 
   102557,100422, 97844, 98121, 97375) #
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
m1 <- (c(
  100000, 99734, 97021, 92607, 93071, 91736)
  / 100000)
qp.start <- 92
m1.name <- 'sim1'
...
m2 <- (c(
  100000, 99519, 96234, 91473, 91946, 90337)
  / 100000) 
m2.name <- 'sim2'

m3.raw <- c(
  100000, 99890, 96310, 91049, 91683, 89443)
m3 <- m3.raw / 100000
m3.name <- 'sim3'

m4.raw <- c(
  100000, 99289, 96063, 91314, 91644, 90074)
m4 <- m4.raw / 100000
m4.name <- 'sim4'

m5 <- (c(
  100000, 99775, 96651, 91870, 92422, 91307)
  / 100000)
m5.name <- 'sim5'  


m6 <- (c(
  100000,100012, 98204, 96007, 96400, 95686)
  / 100000)
m6.name <- 'sim6'  

m7 <- (c(
  100000, 99901, 98262, 96147, 96334, 95340)
  / 100000)
m7.name <- 'sim7'  
...
# Tue
wil5 <- c(0.9586, 1, 0.1308)
nas5 <- c(0.0001, 0.0092, 1)
...
# Thu
wil20 <- c(0.9999, 0.5404, 0.1618)
nas20 <- c(1, 1, 0.9999)

# Fri
nya35 <- c(1, 1, 0)
nas35 <- c(0.9989, 0.1117, 1)
...
(fid.amt <- FidVal[length(FidVal)]) # [86085, 90621]
(fid.pct.in <- w %*% c(
...
(oh.amt <- OHVal[length(OHVal)]) # [97844, 100422]
(oh.pct.in <- w %*% c(
...
# For 401(k): # Next:25 dec (≥10%)
(f1k.total <- f1kVal[length(f1kVal)]) # [153323, 158404] [2-param]
(f1k.pct.in <- w %*% c(
