...
SPActual <- c(df$SPActual,
  ...
  4394,4272,4296,4175,4184,4288,4132)
...
FidVal <- c(df$FidVal,
  ...
  217134,211660,210306,213446,208981) #
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
...
OHIn <- c(df$OHIn,
    ...
    60643, 60643, 61614, 61614, 61614) # 119 + 220 + 937
   ...
   117227,114478,116299,117700,115609) #
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
m1 <- (c(
  100000, 99734, 97021, 92607, 93071, 91736, 92132, 94069, 93531, 90683, 90091,
   ...
   73241, 75371, 71659, 71117, 72945, 70469)
  / 100000)
#qp.start <- 92
m1.name <- 'sim1'
...
m2 <- (c(
  100000, 99519, 96234, 91473, 91946, 90337, 90702, 92795, 92113, 88699, 88134,
   ...
   70160, 71885, 68718, 68253, 70129, 67816)
  / 100000) 
m2.name <- 'sim2'
...
m3.raw <- c(
  100000, 99890, 96310, 91049, 91683, 89443, 89246, 91155, 90781, 88037, 87453, 
   ...
   75824, 77783, 74449, 73896, 75729, 73478)
m3 <- m3.raw / 100000
m3.name <- 'sim3'
...
m4.raw <- c(
  100000, 99289, 96063, 91314, 91644, 90074, 90327, 92360, 91830, 88380, 88025,
   ...
   72509, 74178, 70926, 70529, 72285, 69846)
m4 <- m4.raw / 100000
m4.name <- 'sim4'
...
m5 <- (c(
  100000, 99775, 96651, 91870, 92422, 91307, 92182, 94394, 93768, 89786, 89177,
   ...
   75672, 77615, 73847, 73290, 75208, 72323)
  / 100000)
m5.name <- 'sim5'  
...
m6 <- (c(
  100000,100012, 98204, 96007, 96400, 95686, 96971, 97980, 98605, 95083, 95146,
   ...
   82103, 84200, 80402, 79518, 81849, 78395)
  / 100000)
m6.name <- 'sim6'  
...
m7 <- (c(
  100000, 99901, 98262, 96147, 96334, 95340, 95359, 96898, 96645, 95102, 94744,
   ...
   79338, 81615, 77817, 77332, 78610, 76235)
  / 100000)
m7.name <- 'sim7'  
...
(f1k.total <- f1kVal[length(f1kVal)]) # [  847,   4474] [2-param]
