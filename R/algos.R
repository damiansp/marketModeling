...
sp.actual <- c(
  df$sp.actual,    3824,  3853,  3808,  3895,  3892,  3919,  3970,  3987,  3999,  
     ...
     4146,  4115,  4151,  4205,  4206,  4180,  4221,  4282)
...
fid.in <- c(
  df$fid.in,     161785,161785,161785,161785,161785,161785,161785,162813,162813,
   ...
   166703,166703,166703,166703,167114,167114,167114,167114) #
fid.val <- c(
  df$fid.val,    172350,175073,172978,176119,179159,182181,185241,188878,190980,
   ...
   207316,206749,206649,207834,208340,208551,209575,211305) #
...
f1k.in <- c(
  df$f1k.in,      18031, 18031, 18031, 18031, 18031, 18031, 18031, 18031, 18031,
    ...
    27114, 27114, 27114, 27114, 27114, 27114, 27114, 28080) # (1133.49)
f1k.val <- c(
  df$f1k.val,     17787, 17715, 17856, 17766, 18063, 18219, 18434, 18750, 18928,
    ...
    31526, 31228, 31093, 31424, 31765, 31894, 31822, 33136) #
...
self.mng.val <- c(
  df$self.mng.val, 9189,  9333,  9243,  9351,  9514,  9655,  9818,  9938, 10033,
    ...
    15418, 15321, 15332, 15407, 15517, 15547, 15564, 15647)
...
et.in <- c(
  df$et.in,       76890, 78443, 78443, 78443, 78443, 78443, 78443, 78443, 78443,
    ...
    78521, 78521, 78521, 78521, 78521, 78521, 78521, 78521, 78521, 78521, 78521,
    78521, 78521, 78521, 78521, 78521, 78521, 78521, 78521, 78521, 78521, 78521,
    78521, 78521, 78521, 78521, 78521, 78521, 78521, 78521) #
et.val <-c(
  df$et.val,     123916,128480,126726,128449,130010,131945,134582,135693,137333,
   ...
   150245,149136,147819,148149,147829,148218,149145,150484) #
...
rsi <- (c(
   ...
   109346,108582,107469,108085,108142,107298,110176,111636)
  / 100000)
...
adl <- (c(
   ...
   104824,101934,101607, 97308, 94338, 96310, 98310, 98382, 98739, 94583, 94447,
    ...
    96457, 95170, 94108, 94763, 94518, 94479, 97926, 99235)
  / 100000)
...
(fid.amt <- fid.val[length(fid.val)])                # [179159 211305]
(et.amt <- et.val[length(et.val)])                   # [130010 152885]
(f1k.total <- f1k.val[length(f1k.val)])              # [ 18063  33136] 
(self.mng.amt <- self.mng.val[length(self.mng.val)]) # [  9514  15647]
