...
SPActual <- c(df$SPActual,
  ...
  3921,4024,4072,4130,4119,4091,4155,4152,4145,4140,4122,4210,4207,4280)
...
FidVal <- c(df$FidVal,
  ...
  223126,223592,217642,228217,226121,229954) #
Fid <- FidVal / FidIn
Fid <- Fid / Fid[last]
...
f1kIn <- c(df$f1kIn,
  ...
  0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,0.6935,
    ...
    9837,  9837,  9837,  9837,  9837,  9837) # 961.47
f1kVal <- c(df$f1kVal,
  ...
  147813,146496,145095,141741,141741,141741,     1,     1,     1,     1,     1,
   ...
   11354, 11348, 11318, 11259, 11650, 11567) #
n <- length(f1kIn)
...
OHIn <- c(df$OHIn,
    ...
    69599, 69599, 69599, 69599, 69599, 69599) #
OHVal <-c(df$OHVal,    
   ...
   145537,145944,141846,149554,147699,150552) #
OH <- OHVal / OHIn
OH <- OH / OH[last]
OHd <- dailyChange(OH)
...
m1 <- (c(
  100000, 99734, 97021, 92607, 93071, 91736, 92132, 94069, 93531, 90683, 90091,
   ...
   65788, 66601, 67091, 64831, 67912, 67429, 68755)
  / 100000)
qp.start <- 92
m1.name <- 'sim1'
...
m2 <- (c(
  100000, 99519, 96234, 91473, 91946, 90337, 90702, 92795, 92113, 88699, 88134,
   ...
   64272, 64950, 65338, 63038, 65829, 65264, 66471)
  / 100000) 
m2.name <- 'sim2'
...
m3.raw <- c(
  100000, 99890, 96310, 91049, 91683, 89443, 89246, 91155, 90781, 88037, 87453, 
   ...
   72259, 73000, 72962, 71253, 75030, 73746, 75194)
m3 <- m3.raw / 100000
m3.name <- 'sim3'
...
m4.raw <- c(
  100000, 99289, 96063, 91314, 91644, 90074, 90327, 92360, 91830, 88380, 88025,
   ...
   67060, 67689, 68095, 65872, 68946, 68190, 69814)
m4 <- m4.raw / 100000
m4.name <- 'sim4'
...
m5 <- (c(
  100000, 99775, 96651, 91870, 92422, 91307, 92182, 94394, 93768, 89786, 89177,
   ...
   72179, 73006, 73648, 71269, 74296, 73756, 75021)
  / 100000)
m5.name <- 'sim5'  
...
m6 <- (c(
  100000,100012, 98204, 96007, 96400, 95686, 96971, 97980, 98605, 95083, 95146,
   ...
   76093, 76429, 76635, 74426, 78139, 77354, 79029)
  / 100000)
m6.name <- 'sim6'  
...
m7 <- (c(
  100000, 99901, 98262, 96147, 96334, 95340, 95359, 96898, 96645, 95102, 94744,
   ...
   72950, 73675, 73894, 71681, 75192, 74434, 76055)
  / 100000)
m7.name <- 'sim7'  
...
(oh.amt <- OHVal[length(OHVal)])                     # [92495, 150552] # E*Trade
(f1k.total <- f1kVal[length(f1kVal)])                # [    0,  11567] 
(self.mng.amt <- self.mng.val[length(self.mng.val)]) # [ 5146,   5423]
